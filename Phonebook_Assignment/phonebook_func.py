...
def onSelect(self,event):
    # Calling the event is the self.1sttList1 widget
...
def addToList(self):
    ...
    if not "@" or not "." in var_email: # Will be used later on..
        print("Incorrect email format!!!")
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_email) > 0): # Ensure the user to provide both names and email
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of a fullname, if so we will alert the user and disregard request
            ...
            if chkName == 0: # If this is 0 then there is no existance of the fullname and we can add new data
                ...
                self.lstList1.insert(END, var_fullname) # Update listbox with the full name
                onClear(self) # Call the function to clear all of the textboxes
            else:
                messagebox.showerror("Name Error","'{}' already exists in the database! Please choose a different name.".format(var_fullname))
                onClear(self) # Call the funtion to clear all textboxes
        conn.commit()
        conn.close()
    else:
...
def onDelete(self):
    ...
    with conn:
        ...
        # The database...cannot delete last recod or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmation:", "All information associated with ({}), \nwill be permanently deleted fron the  database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
def onUpdate(self):
    try:
        ...
        messagebox.showinfo("Missing selection","No name selected from the list box. \nCancelling the update request.")
        return
    # The user will only be allowed to update for changes for phone and emails.
    ...
    if (len(var_phone) > 0) and (len(var_email) >0): # Ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            if count == 0 or count2 == 0: # If proposed changges are not already in the database then proceed
                ...
                messagebox.showinfo("No Changes Detected","Both ({}) and ({}) \nalsready exist in the database for this name. \n\nYour update raquest had been cancelled.".format(var_phone,var_email))
            onClear(self)
        conn.close()
    else:
